...
parser.add_argument("-lf", "--frunlength")
...
frunlength = np.float(args.frunlength)
...
#sigma = 2.2868950749000803
#sigma = 2.037391890749937
sigma = 2.0
...
tequil = tau * 100 * 10 #time to run simulation before compression
Ucompress = 0.0001 * (U) #compression speed is in units of swim speed
...
output = 10.0 * 10 * tau / time_step

compresstime = 10 * tau


scratch_path = f"/bigdata2/dan_data/Data/ABP/q12s/"
...
system = hoomd.init.read_gsd(filename=f'ABP_coex_init_{str(frunlength)}.gsd', frame=-1)
...
Vs = particle_volume / np.arange(particle_volume / V, 0.73, 0.0025)
Ls = Vs**(1.0/3.0)
Linitials = Ls[:-1]
Lfinals = Ls[1:]

times = (Linitials - Lfinals) / Ucompress
print(np.sum(times / tequil))
...
#    os.remove(gsd_file)
hoomd.dump.gsd(filename=scratch_path + f'crystal_compress_{str(runlength)}_coex.gsd', period=int(output), group=group_all)

if calc_P:
    logger = hoomd.analyze.log(filename=scratch_path + f'crystal_compress_{str(runlength)}_pressure_coex.log',
...
#hoomd.run(int(tequil / 10 / time_step))
#assert False
...
# run simulation
#hoomd.run(1)
#hoomd.run(int(5 * tequil / time_step))
for i in range(len(Linitials)):
    if i == 0:
        hoomd.run(2 * (tequil / time_step))
    Lvariant = hoomd.variant.linear_interp([(0, Linitials[i]), (int(times[i] / time_step), Lfinals[i])])
