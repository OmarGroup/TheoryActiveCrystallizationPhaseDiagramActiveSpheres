...
# write output
gsd_file = scratch_path + f"ABP_q12_{str(runlength)}_N_{str(N)}_crystal.gsd"
log_output = scratch_path + f"ABP_q12_{str(runlength)}_N_{str(N)}_crystal_pressure.gsd"
...
Vs = particle_volume / np.arange(concf, conc0, 0.0025)[::-1]
Ls = Vs**(1.0/3.0)
Linitials = Ls[:-1]
Lfinals = Ls[1:]
